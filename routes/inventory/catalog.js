...
router.get("/categories", async (req, res) => {
  try {
    ...
    const categories = result
      ...
      .sort((a, b) => b.types.length - a.types.length); // â† most types first
...
router.get("/search", async (req, res) => {
  try {
    ...
    const products = await Catalog.find(filter); // âœ… was Products
    res.json(products);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
router.get("/:id", async (req, res) => {
  try {
    const product = await Catalog.findById(req.params.id); // âœ… was Products
...
router.post("/", async (req, res) => {
  try {
    const product = new Catalog(req.body); // âœ… was Products
...
router.delete("/:id", async (req, res) => {
  try {
    const deletedProduct = await Catalog.findByIdAndDelete(req.params.id); // âœ… was Products
    if (!deletedProduct)
      return res.status(404).json({ message: "Product not found" });
