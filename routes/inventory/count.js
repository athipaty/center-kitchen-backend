...
const PhysicalCount = require("../../models/inventory/PhysicalCount");
const Tag = require("../../models/inventory/Tag");
const SystemStock = require("../../models/inventory/SystemStock");
const Location = require("../../models/inventory/Location");
const ProductionPart = require("../../models/inventory/ProductionPart");
const PreviousDiff = require("../../models/inventory/PreviousDiff");
...
const getProductionSet = async () => {
  try {
    ...
    return new Set(); // âœ… return empty set if anything fails
  }
};
...
router.get("/dashboard-status", async (req, res) => {
  try {
    // ---------- SYSTEM ----------
    const productionSet = await getProductionSet(); // âœ…
    ...
    const actualQtyAgg = await PhysicalCount.aggregate([
      {
        $group: {
          _id: null,
          total: { $sum: "$totalQty" }, // âœ… use totalQty
        },
      },
    ]);
  ...
  try {
    ...
    systemAgg.forEach((s) => {
      if (!productionSet.has(s._id))
        // âœ… exclude production

        systemMap.set(s._id, s.systemQty);
    });

    // âœ… fetch previous diff data
    ...
    actualAgg.forEach((a) => {
      ...
      // âœ… these must be BEFORE the comparison check
    ...
    actualAgg.forEach((a) => {
      ...
      if (systemQty !== undefined && a.totalActual === systemQty) {
        ...
        matched.push({
          ...
          system: systemQty,
          locations: a.locations,
          price: prev?.price ?? null, // âœ…
          diffN1: prev?.diffN1 ?? null, // âœ…
          diffN2: prev?.diffN2 ?? null, // âœ…
        });
      }
    });
...
router.get("/unrecognized", async (req, res) => {
  try {
    ...
    const unrecognized = countedAgg
      ...
      .filter((c) => !productionSet.has(c._id)) // âœ… exclude production
...
router.get("/production-counted", async (req, res) => {
  try {
    ...
    // âœ… only keep parts that ARE in production list
