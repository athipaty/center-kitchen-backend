...
router.post('/upload-mapping', upload.single('file'), async (req, res) => {
  try {
    ...
    const mapping = rows.map(r => ({
      stockPartNo: (r['stock_part_no'] || r['Stock Part No'] || r['stock part no'] || Object.values(r)[0] || '').toString().trim(),
      systemPartNo: (r['system_part_no'] || r['System Part No'] || r['system part no'] || Object.values(r)[1] || '').toString().trim(),
...
router.post('/upload-stock', upload.single('file'), async (req, res) => {
  try {
    ...
    const mapLookup = {};
    stockData.mapping.forEach(m => mapLookup[m.stockPartNo.toLowerCase()] = m.systemPartNo);

    const { type } = req.body; // 'current', 'incoming', 'po', 'forecast'

    if (type === 'current') {
      stockData.currentStock = rows.map(r => {
        ...
        return { partNo, qty: parseQty(r['qty'] || r['Qty'] || r['stock'] || Object.values(r)[1]) };
