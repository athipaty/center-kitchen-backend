...
function normaliseRows(rawRows) {
  const skip = new Set(['customer', 'part no', 'upload date', 'uploaddate']);
  ...
  return rawRows.map(r => {
    const customer =
      r['customer'] || r['Customer'] || r['CUSTOMER'] || '';
    const partNo =
...
router.get('/compare', async (req, res) => {
  try {
    ...
    [...prev.rows, ...curr.rows].forEach(r => {
      Object.keys(r.quantities.toJSON ? r.quantities.toJSON() : r.quantities).forEach(m =>
        monthSet.add(m)
      );
    });
    ...
    const rows = allKeys.map(key => {
      ...
      const c = currMap[key];
      const base = c || p;
      const pQty = p ? (p.quantities.toJSON ? p.quantities.toJSON() : p.quantities) : {};
      const cQty = c ? (c.quantities.toJSON ? c.quantities.toJSON() : c.quantities) : {};

      const monthData = months.map(m => {
        const pv = pQty[m] || 0;
        const cv = cQty[m] || 0;
        const diff = cv - pv;
        const pct = pv === 0 ? (cv > 0 ? 100 : 0) : Math.round((diff / pv) * 100);
        return { month: m, prev: pv, curr: cv, diff, pct, alert: Math.abs(pct) >= 20 && (pv > 0 || cv > 0) };
      });
    ...
    res.json({ months, rows, prevFile: prev.filename, currFile: curr.filename });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: err.message });
  }
